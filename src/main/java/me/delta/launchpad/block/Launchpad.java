...
public class Launchpad extends Block implements BlockEntityProvider, PolymerBlock {
    ...
    public void onEntityCollision(BlockState state,
        ...
        if (blockEntity instanceof LaunchPadBlockEntity launchPad)
        {
            tryLaunch(entity,
                    ...
                    launchPad.getPitch().orElse(Math.max(-entity.getPitch(0), 5)), //add 5 to the pitch to avoid player spamming the launchpad
                    launchPad.getYaw().orElse(entity.getYaw(0)),
                    launchPad.getPower());
        }
    ...
    public static void tryLaunch(Entity entity, Entity source, SoundEvent sound, SoundCategory category, float pitch, float yaw, float power)
    {
        if (entity.isOnGround() && !(entity instanceof ArmorStandEntity))
        {
            entity.setVelocity(getVector(pitch, yaw).multiply(power));
